...
{
    /// <summary>
    /// 使用本地登录。这个注入IAuth时，只需要SysCommon.Mvc一个项目即可，无需webapi的支持
    /// </summary>
    public class LocalAuth :IAuth
    {
        ...
        public AuthStrategyContext GetCurrentUser()
        {
            ...
            if (user != null)
            {
                context = _app.GetAuthStrategyContext(user.Name);
            }
            return context;
        }
        ...
        public LoginResult Login(PassportLoginRequest passportLoginRequest)
        {
            if (_appConfiguration.Value.IsIdentityAuth)
            {
                ...
                {
                    Code = 101,
                    Message = ApiMessage.IdentityAuth
                };
            }
            var result = _loginParse.Do(new PassportLoginRequest
            {
                //AppKey = appKey,
                UserName = passportLoginRequest.UserName,
                Password = passportLoginRequest.Password,
                Uuid = passportLoginRequest.Uuid,
                VerificationCode = passportLoginRequest.VerificationCode
            });

            var log = new SysLog
            {
                Content = $"用户登录,结果：{result.Message}",
                Result = result.Code == 100 ? 0 : 1,
                CreateName = passportLoginRequest.UserName,
                TypeName = "登录日志",
                Application = "121",
                CreateId = "12",
                Description = "12",
