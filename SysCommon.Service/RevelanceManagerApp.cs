...
{
    public class RevelanceManagerApp  :BaseService<Relevance>
    {
        ...
        private void DeleteBy(string key, ILookup<string, string> idMaps)
        {
            foreach (var sameVals in idMaps)
            {
                ...
                {
                    Repository.Delete(u => u.TopKey == key && u.TopFirstId == sameVals.Key && u.TopFirstId == value);
                }
            }
        }
        ...
        {
            Repository.Delete(u => firstIds.Contains(u.TopFirstId) && u.TopKey == key);
        }
        ...
        public List<string> Get(string key, bool returnSecondIds, params string[] ids)
        {
            if (returnSecondIds)
            {
                return Repository.Find(u => u.TopKey == key
                                                      && ids.Contains(u.TopFirstId)).Select(u => u.TopFirstId).ToList();
            }
            else
            {
                return Repository.Find(u => u.TopKey == key
        ...
        {
            return Repository.Find(u => u.TopKey == key && u.TopFirstId == firstId && u.TopFirstId == secondId)
                .Select(u => u.TopFirstId).ToList();
        }
        ...
        public void UnAssignData(AssignDataReq request)
        {
            ...
            else  //按具体的id删除
            {
                ...
                {
                    Repository.Delete(u => u.TopKey == Define.ROLEDATAPROPERTY
                                           && u.TopFirstId == request.RoleId 
                                           && u.TopFirstId == request.ModuleCode 
                                           && u.TopFirstId == property);
                }
            }
        }
        ...
        {
            //删除以前的所有用户
            UnitWork.Delete<Relevance>(u => u.TopFirstId == request.RoleId && u.TopKey == Define.USERROLE);
            //批量分配用户角色
        ...
        {
            //删除以前的所有用户
            UnitWork.Delete<Relevance>(u => u.TopFirstId == request.OrgId && u.TopKey == Define.USERORG);
            //批量分配用户角色
            UnitWork.BatchAdd((from firstId in request.UserIds
                select new Relevance
                {
