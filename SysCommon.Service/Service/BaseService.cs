...
namespace SysCommon.Service
{
    ...
    public class BaseService<T> where T : Entity
    {
        ...
        public BaseService(IUnitWork unitWork, IRepository<T> repository, IAuth auth)
        {
            UnitWork = unitWork;
        ...
        protected IQueryable<T> GetDataPrivilege(string parametername)
        {
            ...
            if (loginUser.User.UserName == Define.SYSTEM_USERNAME) return UnitWork.Find<T>(null);  //超级管理员特权
