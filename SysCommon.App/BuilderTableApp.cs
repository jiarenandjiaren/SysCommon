...
{
    public class BuilderTableApp : BaseApp<BuilderTable,SysCommonDBContext>
    {
        ...
        public BuilderTableApp(IUnitWork<SysCommonDBContext> unitWork, IRepository<BuilderTable,SysCommonDBContext> repository,
        ...
        public void CreateVue(CreateVueReq req)
        {
            ...
            {
                throw new Exception("请提供vue项目的根目录,如：C:\\SysCommon.Pro\\Client");
            }
            var sysTableInfo = Repository.FirstOrDefault(u => u.Id == req.Id);
            var tableColumns = _builderTableColumnApp.Find(req.Id);
            if (sysTableInfo == null
