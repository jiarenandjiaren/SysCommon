...
// CONCATENATED MODULE: ./node_modules/cache-loader/dist/cjs.js?{"cacheDirectory":"node_modules/.cache/vue-loader","cacheIdentifier":"1eb40bb3-vue-loader-template"}!./node_modules/vue-loader/lib/loaders/templateLoader.js??vue-loader-options!./node_modules/cache-loader/dist/cjs.js??ref--0-0!./node_modules/vue-loader/lib??vue-loader-options!./src/components/vue-draggable-resizable.vue?vue&type=template&id=1c582b35&
var render = function () {
var _obj;
...
// CONCATENATED MODULE: ./src/components/vue-draggable-resizable.vue?vue&type=template&id=1c582b35&
...
/* harmony default export */ var vue_draggable_resizablevue_type_script_lang_js_ = ({
  ...
  created: function created() {
    ...
    if (this.maxWidth && this.minWidth > this.maxWidth) console.warn('[Vdr warn]: Invalid prop: minWidth cannot be greater than maxWidth'); // eslint-disable-next-line 无效prop：minHeight不能大于maxHeight'
  ...
  methods: {
    ...
    checkParentSize: function checkParentSize() {
      if (this.parent) {
        var _this$getParentSize3 = this.getParentSize(),
            ...
            newParentHeight = _this$getParentSize4[1]; // 修复父元素改变大小后，组件resizing时活动异常
    ...
    getParentSize: function getParentSize() {
      ...
      if (typeof this.parent === 'string') {
        var parentNode = document.querySelector(this.parent);

        if (!(parentNode instanceof HTMLElement)) {
          throw new Error("The selector ".concat(this.parent, " does not match any element"));
        }
    ...
    // 元素移动
    handleDrag: function () {
      var _handleDrag = _asyncToGenerator(
      /*#__PURE__*/
      regeneratorRuntime.mark(function _callee(e) {
        var axis, grid, bounds, mouseClickPosition, tmpDeltaX, tmpDeltaY, _snapToGrid, _snapToGrid2, deltaX, deltaY, left, top, right, bottom;
          ...
          while (1) {
            switch (_context.prev = _context.next) {
              case 0:
                ...
                grid = this.grid;
                bounds = this.bounds;
                mouseClickPosition = this.mouseClickPosition;
                tmpDeltaX = axis && axis !== 'y' ? mouseClickPosition.mouseX - (e.touches ? e.touches[0].pageX : e.pageX) : 0;
                tmpDeltaY = axis && axis !== 'x' ? mouseClickPosition.mouseY - (e.touches ? e.touches[0].pageY : e.pageY) : 0;
                _snapToGrid = snapToGrid(grid, tmpDeltaX, tmpDeltaY, this.scaleRatio), _snapToGrid2 = _slicedToArray(_snapToGrid, 2), deltaX = _snapToGrid2[0], deltaY = _snapToGrid2[1];
    ...
    handleResize: function handleResize(e) {
      ...
      var _snapToGrid7 = snapToGrid(this.grid, tmpDeltaX, tmpDeltaY, this.scaleRatio),
    ...
    changeWidth: function changeWidth(val) {
      var _snapToGrid9 = snapToGrid(this.grid, val, 0, this.scale),
    ...
    changeHeight: function changeHeight(val) {
      var _snapToGrid11 = snapToGrid(this.grid, 0, val, this.scale),
    ...
    // 从控制柄松开
    handleUp: function () {
      var _handleUp = _asyncToGenerator(
      /*#__PURE__*/
      regeneratorRuntime.mark(function _callee2(e) {
        var temArr, refLine, i;
        return regeneratorRuntime.wrap(function _callee2$(_context2) {
          while (1) {
            switch (_context2.prev = _context2.next) {
              case 0:
                this.handle = null; // 初始化辅助线数据

                temArr = new Array(3).fill({
                  display: false,
                  position: '',
                  origin: '',
                  lineLength: ''
    ...
    // 新增方法 ↓↓↓
    // 设置属性
    settingAttribute: function settingAttribute() {
      // 设置冲突检测
      this.$el.setAttribute('data-is-check', "".concat(this.isConflictCheck)); // 设置对齐元素

      this.$el.setAttribute('data-is-snap', "".concat(this.snap));
    },
    // 冲突检测
    conflictCheck: function conflictCheck() {
      ...
      if (this.isConflictCheck) {
        var nodes = this.$el.parentNode.childNodes; // 获取当前父节点下所有子节点
        ...
        try {
          for (var _iterator = nodes[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
            ...
            if (item.className !== undefined && !item.className.includes(this.classNameActive) && item.getAttribute('data-is-check') !== null && item.getAttribute('data-is-check') !== 'false') {
              var tw = item.offsetWidth;
              var th = item.offsetHeight; // 正则获取left与right

              var _this$formatTransform = this.formatTransformVal(item.style.transform),
                  _this$formatTransform2 = _slicedToArray(_this$formatTransform, 2),
                  tl = _this$formatTransform2[0],
                  tt = _this$formatTransform2[1]; // 左上角与右下角重叠


              var tfAndBr = top >= tt && left >= tl && tt + th > top && tl + tw > left || top <= tt && left < tl && top + height > tt && left + width > tl; // 右上角与左下角重叠

              var brAndTf = left <= tl && top >= tt && left + width > tl && top < tt + th || top < tt && left > tl && top + height > tt && left < tl + tw; // 下边与上边重叠

              var bAndT = top <= tt && left >= tl && top + height > tt && left < tl + tw || top >= tt && left <= tl && top < tt + th && left > tl + tw; // 上边与下边重叠（宽度不一样）

              var tAndB = top <= tt && left >= tl && top + height > tt && left < tl + tw || top >= tt && left <= tl && top < tt + th && left > tl + tw; // 左边与右边重叠

              var lAndR = left >= tl && top >= tt && left < tl + tw && top < tt + th || top > tt && left <= tl && left + width > tl && top < tt + th; // 左边与右边重叠（高度不一样）

              var rAndL = top <= tt && left >= tl && top + height > tt && left < tl + tw || top >= tt && left <= tl && top < tt + th && left + width > tl; // 如果冲突，就将回退到移动前的位置
    ...
    // 检测对齐元素
    snapCheck: function () {
      var _snapCheck = _asyncToGenerator(
      /*#__PURE__*/
      regeneratorRuntime.mark(function _callee3() {
        var width, height, activeLeft, activeRight, activeTop, activeBottom, temArr, refLine, i, nodes, tem, _ref, groupWidth, groupHeight, groupLeft, groupTop, bln, _iteratorNormalCompletion2, _didIteratorError2, _iteratorError2, _iterator2, _step2, item, w, h, _this$formatTransform3, _this$formatTransform4, l, t, r, b, hc, vc, ts, TS, bs, BS, ls, LS, rs, RS, arrTem, _i, xory, horv, _this$calcLineValues, origin, length;
          ...
          while (1) {
            switch (_context3.prev = _context3.next) {
              case 0:
                ...
                activeTop = this.top;
                activeBottom = this.top + height; // 初始化辅助线数据
                ...
                nodes = this.$el.parentNode.childNodes;
                tem = {
                  value: {
                    x: [[], [], []],
                    y: [[], [], []]
                ...
                _context3.next = 14;
                return this.getActiveAll(nodes);

              case 14:
                _ref = _context3.sent;
                groupWidth = _ref.groupWidth;
                groupHeight = _ref.groupHeight;
                groupLeft = _ref.groupLeft;
                groupTop = _ref.groupTop;
                bln = _ref.bln;
                ...
                for (_iterator2 = nodes[Symbol.iterator](); !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
                  ...
                  if (item.className !== undefined && !item.className.includes(this.classNameActive) && item.getAttribute('data-is-snap') !== null && item.getAttribute('data-is-snap') !== 'false') {
                    w = item.offsetWidth;
                    h = item.offsetHeight;
                    _this$formatTransform3 = this.formatTransformVal(item.style.transform), _this$formatTransform4 = _slicedToArray(_this$formatTransform3, 2), l = _this$formatTransform4[0], t = _this$formatTransform4[1];
                    r = l + w; // 对齐目标right

                    b = t + h; // 对齐目标的bottom

                    hc = Math.abs(activeTop + height / 2 - (t + h / 2)) <= this.snapTolerance; // 水平中线

                    vc = Math.abs(activeLeft + width / 2 - (l + w / 2)) <= this.snapTolerance; // 垂直中线

                    ts = Math.abs(t - activeBottom) <= this.snapTolerance; // 从上到下

                    TS = Math.abs(b - activeBottom) <= this.snapTolerance; // 从上到下

                    bs = Math.abs(t - activeTop) <= this.snapTolerance; // 从下到上

                    BS = Math.abs(b - activeTop) <= this.snapTolerance; // 从下到上

                    ls = Math.abs(l - activeRight) <= this.snapTolerance; // 外左

                    LS = Math.abs(r - activeRight) <= this.snapTolerance; // 外左

                    rs = Math.abs(l - activeLeft) <= this.snapTolerance; // 外右

                    RS = Math.abs(r - activeLeft) <= this.snapTolerance; // 外右
                    ...
                    arrTem = [0, 1, 0, 1, 2, 2, 0, 1, 0, 1, 2, 2];

                    for (_i = 0; _i <= arrTem.length; _i++) {
                      // 前6为Y辅助线,后6为X辅助线
                      xory = _i < 6 ? 'y' : 'x';
                      horv = _i < 6 ? 'hLine' : 'vLine';

                      if (tem.display[_i]) {
                        _this$calcLineValues = this.calcLineValues(tem.value[xory][arrTem[_i]]), origin = _this$calcLineValues.origin, length = _this$calcLineValues.length;
                        refLine[horv][arrTem[_i]].display = tem.display[_i];
                        refLine[horv][arrTem[_i]].position = tem.position[_i] + 'px';
                        refLine[horv][arrTem[_i]].origin = origin;
                        refLine[horv][arrTem[_i]].lineLength = length;
                      }
                    }
                  }
                }
      ...
      function snapCheck() {
        return _snapCheck.apply(this, arguments);
      }

      return snapCheck;
    }(),
    calcLineValues: function calcLineValues(arr) {
      var length = Math.max.apply(Math, _toConsumableArray(arr)) - Math.min.apply(Math, _toConsumableArray(arr)) + 'px';
      var origin = Math.min.apply(Math, _toConsumableArray(arr)) + 'px';
      return {
        length: length,
        origin: origin
      };
    },
    getActiveAll: function () {
      var _getActiveAll = _asyncToGenerator(
      /*#__PURE__*/
      regeneratorRuntime.mark(function _callee4(nodes) {
        var activeAll, XArray, YArray, groupWidth, groupHeight, groupLeft, groupTop, _iteratorNormalCompletion3, _didIteratorError3, _iteratorError3, _iterator3, _step3, item, AllLength, _iteratorNormalCompletion4, _didIteratorError4, _iteratorError4, _iterator4, _step4, i, l, r, t, b, bln;
          ...
          while (1) {
            switch (_context4.prev = _context4.next) {
              ...
              case 26:
                ...
                for (_iterator4 = activeAll[Symbol.iterator](); !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
                  i = _step4.value;
                  l = i.offsetLeft;
                  r = l + i.offsetWidth;
                  t = i.offsetTop;
              ...
              case 47:
                groupWidth = Math.max.apply(Math, YArray) - Math.min.apply(Math, YArray);
                groupHeight = Math.max.apply(Math, XArray) - Math.min.apply(Math, XArray);
                groupLeft = Math.min.apply(Math, YArray);
                groupTop = Math.min.apply(Math, XArray);
        ...
        }, _callee4, this, [[10, 14, 18, 26], [19,, 21, 25], [31, 35, 39, 47], [40,, 42, 46]]);
      }));

      function getActiveAll(_x3) {
        return _getActiveAll.apply(this, arguments);
      }

      return getActiveAll;
    }(),
    // 正则获取left与top
    formatTransformVal: function formatTransformVal(string) {
      var _string$replace$split = string.replace(/[^0-9\-,]/g, '').split(','),
          _string$replace$split2 = _slicedToArray(_string$replace$split, 2),
          left = _string$replace$split2[0],
          top = _string$replace$split2[1];
  ...
  computed: {
    handleStyle: function handleStyle() {
      ...
      return function (stick) {
        if (!_this.handleInfo.switch) return {
          display: _this.enabled ? 'block' : 'none'
        };

        var size = (_this.handleInfo.size / _this.scaleRatio).toFixed(2);

        var offset = (_this.handleInfo.offset / _this.scaleRatio).toFixed(2);

        var center = (size / 2).toFixed(2);
        var styleMap = {
          tl: {
          ...
          bm: {
            bottom: "".concat(offset, "px"),
            right: "calc(50% - ".concat(center, "px)")
          },
          bl: {
            bottom: "".concat(offset, "px"),
            left: "".concat(offset, "px")
        ...
        stickStyle.display = _this.enabled ? 'block' : 'none';
        return stickStyle;
      };
    },
